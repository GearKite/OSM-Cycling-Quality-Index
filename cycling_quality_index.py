...
project_dir = (
    os.path.dirname(_console.console.tabEditorWidget.currentWidget().path) + "/"
)
...
default_width_traffic_lane = (
    3.2  # average width of a driving lane (for common motor cars)
)
...
default_cycleway_surface_tracks = "asphalt"  # common surface on cycle tracks
...
maxspeed_factor_dict = {20: 1.05, 30: 1, 50: 0.95, 60: 0.85, 70: 0.7, 100: 0.5}
...
transform_context.addCoordinateOperation(
    QgsCoordinateReferenceSystem(crs_from), QgsCoordinateReferenceSystem(crs_to), ""
)
coordinateTransformContext = QgsProject.instance().transformContext()
save_options = QgsVectorFileWriter.SaveVectorOptions()
save_options.driverName = "GeoJSON"
save_options.ct = QgsCoordinateTransform(
    QgsCoordinateReferenceSystem(crs_from),
    QgsCoordinateReferenceSystem(crs_to),
...
def deriveAttribute(feature, attribute_name, type, side, vartype):
    ...
    attribute = feature.attribute(
        str(type) + ":" + str(side) + ":" + str(attribute_name)
    )
    if not attribute:
...
def deriveSeparation(feature, traffic_mode):
    ...
    if traffic_mode == "motor_vehicle":
        if traffic_mode_right in ["motor_vehicle", "parking", "psv"]:
            separation = separation_right
        if not traffic_mode_left or traffic_mode_left in [
            "motor_vehicle",
            "parking",
            "psv",
        ]:
...
def getAccess(feature, access_key):
    ...
    if not access_value and access_key in access_dict:
        for i in range(len(access_dict[access_key])):
            if (
                not access_value
                and feature.fields().indexOf(access_dict[access_key][i]) != -1
            ):
    ...
    for i in range(len(value_list)):
        if value_list[i] in surface_value_list:
            if not value:
                ...
                if surface_value_list.index(value_list[i]) > surface_value_list.index(
                    value
                ):
                    value = value_list[i]
    return value
...
if multi_input:
    ...
    while exists(dir_input + str(i) + file_format):
        ...
        layer_way_input = processing.run(
            "native:retainfields",
            {"INPUT": layer_way_input, "FIELDS": attributes_list, "OUTPUT": "memory:"},
        )["OUTPUT"]
...
if not exists(dir_input + file_format):
    if multi_input:
        ...
        print(
            time.strftime("%H:%M:%S", time.localtime()),
            '[!] Error: No valid input file at "' + dir_input + file_format + '".',
        )
else:
    layer_way_input = QgsVectorLayer(
    ...
    layer = processing.run(
        "native:reprojectlayer",
        {
            ...
            "TARGET_CRS": QgsCoordinateReferenceSystem(crs_to),
            "OUTPUT": "memory:",
        },
    )["OUTPUT"]
    ...
    id_prot_level_separation_right = layer.fields().indexOf(
        "prot_level_separation_right"
    )
    ...
    sidepath_buffer_distance = (
        100  # do checks for adjacent roads every ... meters along a way
    )
    ...
    layer_path_points_endpoints = processing.run(
        ...
        {"INPUT": layer_path, "VERTICES": "-1", "OUTPUT": "memory:"},
    )["OUTPUT"]
    layer_path_points = processing.run(
    ...
    for buffer in layer_path_points_buffers.getFeatures():
        ...
        for road in layer_roads.selectedFeatures():
            ...
            if (
                not road_highway in maxspeed_dict
        ...
        for highway in maxspeed_dict.keys():
            if (
                not highway in sidepath_dict[buffer_id]["maxspeed"]
    ...
    with edit(layer):
        for feature in layer.getFeatures():
            ...
            if highway != "cycleway":
                ...
                if (
                    ...
                    layer.changeAttributeValue(
                        feature.id(), id_offset_cycleway_left, offset_cycleway_left
                    )
                ...
                if (
                    ...
                    layer.changeAttributeValue(
                        feature.id(), id_offset_cycleway_right, offset_cycleway_right
                    )
            ...
            if (
                ...
                layer.changeAttributeValue(
                    feature.id(), id_offset_sidewalk_left, offset_sidewalk_left
                )
            ...
            if (
                ...
                layer.changeAttributeValue(
                    feature.id(), id_offset_sidewalk_right, offset_sidewalk_right
                )
        ...
        offset_cycleway_left_layer = processing.run(
            "native:offsetline",
            {
        ...
        offset_cycleway_right_layer = processing.run(
            "native:offsetline",
            {
                "INPUT": QgsProcessingFeatureSourceDefinition(
                    layer.id(), selectedFeaturesOnly=True
        ...
        offset_sidewalk_left_layer = processing.run(
            "native:offsetline",
            {
                "INPUT": QgsProcessingFeatureSourceDefinition(
                    layer.id(), selectedFeaturesOnly=True
        ...
        offset_sidewalk_right_layer = processing.run(
            "native:offsetline",
            {
                "INPUT": QgsProcessingFeatureSourceDefinition(
                    layer.id(), selectedFeaturesOnly=True
        ...
        for type in ["cycleway", "sidewalk"]:
            ...
            with edit(offset_layer):
                for feature in offset_layer.getFeatures():
                    ...
                    offset_layer.changeAttributeValue(
                        feature.id(), id_proc_sidepath, "yes"
                    )
                    offset_layer.changeAttributeValue(
                        feature.id(), id_proc_highway, feature.attribute("highway")
                    )
                    offset_layer.changeAttributeValue(
                        feature.id(), id_proc_maxspeed, feature.attribute("maxspeed")
                    )

                    offset_layer.changeAttributeValue(
                        feature.id(),
                        offset_layer.fields().indexOf("width"),
                        deriveAttribute(feature, "width", type, side, "float"),
                    )
                    offset_layer.changeAttributeValue(
                        feature.id(),
                        offset_layer.fields().indexOf("oneway"),
                        deriveAttribute(feature, "oneway", type, side, "str"),
                    )
                    offset_layer.changeAttributeValue(
                        feature.id(),
                        offset_layer.fields().indexOf("oneway:bicycle"),
                        deriveAttribute(feature, "oneway:bicycle", type, side, "str"),
                    )
                    offset_layer.changeAttributeValue(
                        feature.id(),
                        offset_layer.fields().indexOf("traffic_sign"),
                        deriveAttribute(feature, "traffic_sign", type, side, "str"),
                    )
                    ...
                    if type != "cycleway" or (
                        ...
                        offset_layer.changeAttributeValue(
                            ...
                            deriveAttribute(feature, "surface", type, side, "str"),
                        )
                    if type != "cycleway" or (
                        type == "cycleway"
                        and (
                        ...
                        offset_layer.changeAttributeValue(
                            ...
                            deriveAttribute(feature, "smoothness", type, side, "str"),
                        )
                    ...
                    if type == "cycleway":
                        offset_layer.changeAttributeValue(
                            ...
                            deriveAttribute(feature, "separation", type, side, "str"),
                        )
                        offset_layer.changeAttributeValue(
                            feature.id(),
                        ...
                        offset_layer.changeAttributeValue(
                            ...
                            deriveAttribute(feature, "buffer", type, side, "str"),
                        )
                        offset_layer.changeAttributeValue(
                            feature.id(),
                            offset_layer.fields().indexOf("buffer:both"),
                            deriveAttribute(feature, "buffer:both", type, side, "str"),
                        )
                        offset_layer.changeAttributeValue(
                            feature.id(),
                            offset_layer.fields().indexOf("buffer:left"),
                            deriveAttribute(feature, "buffer:left", type, side, "str"),
                        )
                        offset_layer.changeAttributeValue(
                            feature.id(),
                            offset_layer.fields().indexOf("buffer:right"),
                            deriveAttribute(feature, "buffer:right", type, side, "str"),
                        )
    ...
    layer = processing.run(
        "native:mergevectorlayers",
        {
            "LAYERS": [
                layer,
                offset_cycleway_left_layer,
                offset_cycleway_right_layer,
                offset_sidewalk_left_layer,
                offset_sidewalk_right_layer,
            ],
            "OUTPUT": "memory:",
        },
    )["OUTPUT"]
    ...
    with edit(layer):
        for feature in layer.getFeatures():
            ...
            if getAccess(feature, "bicycle") and getAccess(feature, "bicycle") not in [
                "yes",
                "permissive",
                "designated",
                "use_sidepath",
            ...
            if way_type == "":
                ...
                if highway in ["footway", "pedestrian", "bridleway", "steps"]:
                    if bicycle not in ["no", "dismount", "use_sidepath"]:
                    ...
                    if foot in ["yes", "designated", "permissive"]:
                        ...
                        if separation_foot == "no":
                            ...
                            if not is_sidepath in ["yes", "no"]:
                                ...
                                separation_motor_vehicle = deriveSeparation(
                        ...
                        if type == "sidewalk":
                            ...
                            if (
                                ...
                                if cycleway_lanes and "no|lane|no" in cycleway_lanes:
                                    ...
                                    if not separation_motor_vehicle in [
                                        NULL,
                                        "no",
                                        "none",
                                    ]:
                            ...
                            # for cycle tracks
                                ...
                                if (
                                    ...
                                    if (
                                        ...
                                        separation_foot = deriveSeparation(
                                            feature, "foot"
                                        )
                                        if separation_foot == "no":
                                            way_type = "segregated path"
        ...
        for feature in layer.getFeatures():
            ...
            if way_type in [
                ...
                if way_type in ["shared traffic lane", "shared bus lane"]:
                    ...
                    if (
                        ...
                        proc_width = getNumber(
                            width_lanes[width_lanes.rfind("|") + 1 :]
                        )
                    elif (
                        (way_type == "shared bus lane" and not "yes" in proc_oneway)
                        and side == "right"
                        ...
                        if way_type == "shared bus lane":
                            ...
                            data_missing = addDelimitedValue(
                ...
                if not proc_width:
                    ...
                    if not proc_width:
                        ...
                        if parking_right == "lane" or parking_right == "half_on_kerb":
                            if not parking_right_width:
                                if parking_right_orientation == "diagonal":
                                    ...
                                    parking_right_width = (
                                        default_width_parking_perpendicular
                                    )
                                else:
                        ...
                        buffer = (
                            getNumber(cycleway_right_buffer_left)
                            + getNumber(cycleway_right_buffer_right)
                            + getNumber(cycleway_left_buffer_left)
                            + getNumber(cycleway_left_buffer_right)
                        )
                        proc_width = (
                            width
                            - getNumber(cycleway_right_width)
                            - getNumber(cycleway_left_width)
                            - buffer
                        )
                        ...
                        if parking_right or parking_left:
                            proc_width = (
                                proc_width
                                - getNumber(parking_right_width)
                                - getNumber(parking_left_width)
                            )
            ...
            if surface_bicycle:
                if surface_bicycle in surface_factor_dict:
                    ...
                    proc_surface = getWeakestSurfaceValue(
                        getDelimitedValues(surface_bicycle, ";", "string")
                    )
            ...
            if ";" in proc_surface:
                proc_surface = getWeakestSurfaceValue(
                    getDelimitedValues(proc_surface, ";", "string")
                )
            ...
            layer.changeAttributeValue(
                feature.id(), id_proc_smoothness, proc_smoothness
            )
            ...
            layer.changeAttributeValue(
                feature.id(), id_proc_traffic_mode_left, traffic_mode_left
            )
            layer.changeAttributeValue(
                feature.id(), id_proc_traffic_mode_right, traffic_mode_right
            )
            layer.changeAttributeValue(
                feature.id(), id_proc_separation_left, separation_left
            )
            layer.changeAttributeValue(
                feature.id(), id_proc_separation_right, separation_right
            )
            ...
            if way_type in [
                ...
                if is_sidepath == "yes":
                    ...
                    if traffic_sign:
                        traffic_sign = getDelimitedValues(
            ...
            layer.changeAttributeValue(
                feature.id(), id_proc_traffic_sign, proc_traffic_sign
            )
            ...
            layer.changeAttributeValue(
                feature.id(), id_filter_way_type, filter_way_type
            )
            ...
            if (
                ...
                or getAccess(feature, "motor_vehicle") == "no"
            ):
            ...
            if calc_width:
                ...
                if (
                    ...
                    and motor_vehicle_access in motor_vehicle_access_index_dict
                ):
            ...
            if way_type in [
                ...
                if (
                    way_type == "bicycle road"
                    and getAccess(feature, "motor_vehicle")
                    in motor_vehicle_access_index_dict
                ):
                    lts = 1
                else:
            ...
            layer.changeAttributeValue(
                feature.id(), id_data_incompleteness, data_incompleteness
            )
    ...
    layer = processing.run(
        ...
        {
            "INPUT": layer,
            "FIELDS": attributes_list_finally_retained,
            "OUTPUT": "memory:",
        },
    )["OUTPUT"]
